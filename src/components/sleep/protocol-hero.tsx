...
import { Zap, Pill, Activity, ChevronRight } from "lucide-react";
...
export function ProtocolHero({ protocol }: ProtocolHeroProps) {
  ...
  return (
    <div className="card elevation-1 p-4 relative overflow-hidden">
      ...
      <div className="relative">
        <div className="flex items-center gap-2 mb-3">
          ...
          {stackName && (
            <span className="text-[8px] px-1.5 py-0.5 rounded-full bg-purple-500/15 text-purple-400 border border-purple-500/20 ml-auto">
              ≈ {stackName}
            </span>
          )}
        </div>
        ...
        {protocol.supplements.length > 0 && (
          <div className="mb-3">
            <div className="flex items-center gap-1.5 mb-1.5">
              <Pill className="h-2.5 w-2.5 text-text-muted" />
              <span className="text-[9px] uppercase tracking-wider text-text-muted font-semibold">Supplements</span>
            </div>
            <div className="space-y-1">
              {protocol.supplements.map((item) => (
                <div key={item.id} className="flex items-center gap-2 group">
                  ...
                  <span className="text-[11px] text-text-secondary font-medium flex-1 min-w-0 truncate">
                    {item.name}
                  </span>
                  <span className="text-[9px] text-text-muted font-data">{item.timing}</span>
        ...
        {protocol.interventions.length > 0 && (
          <div className="mb-3">
            <div className="flex items-center gap-1.5 mb-1.5">
              ...
              <span className="text-[9px] uppercase tracking-wider text-text-muted font-semibold">Interventions</span>
            </div>
            <div className="space-y-1">
        ...
        <div className="flex items-center justify-between pt-2 border-t border-border-default/50">
          <div className="flex items-center gap-2">
            <span className="text-[9px] text-text-muted uppercase tracking-wider">Predicted quality</span>
